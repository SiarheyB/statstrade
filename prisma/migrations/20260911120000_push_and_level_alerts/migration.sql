-- Источник событий экономического календаря. Всё, что уже лежит в таблице,
-- пришло из ForexFactory — поэтому DEFAULT и есть корректное значение для
-- существующих строк, отдельного UPDATE не требуется.
ALTER TABLE "EconomicEvent" ADD COLUMN "source" TEXT NOT NULL DEFAULT 'forexfactory';

-- Ключ расширяем источником: одно и то же событие приходит и из ForexFactory,
-- и из investing (с другим написанием и другой важностью). Старый ключ
-- (time, currency, title) заставил бы их затирать друг друга, и переключение
-- источника в админке превращалось бы в порчу данных.
DROP INDEX IF EXISTS "EconomicEvent_time_currency_title_key";
CREATE UNIQUE INDEX "EconomicEvent_source_time_currency_title_key"
    ON "EconomicEvent"("source", "time", "currency", "title");
CREATE INDEX "EconomicEvent_source_time_idx" ON "EconomicEvent"("source", "time");
