...
| **Проблема** | ❌ **Не доступен из РБ** (JForex/демо-счёт блокируются) |

**➡️ Заменён на Twelve Data REST API (см. ниже)**

> **Уточнение от 2026-08-21.** Из РБ недоступен именно JForex (Java API + демо-счёт).
> Обычный HTTP-фид Dukascopy — `freeserv.dukascopy.com` (бэкенд их бесплатных
> виджетов) — с белорусского IP **работает**: проверено прямо на прод-сервере,
> HTTP 200, свечи 1MIN по XAU/USD. Ключ не нужен, но **обязательны заголовки
> `User-Agent` и `Referer`** — без них сервис отдаёт 200 с пустым телом, и это
> легко принять за блокировку по региону.
>
> Отсюда золото (XAU/USD) и история таймфрейма 1m берутся именно из Dukascopy:
> см. `collector/forex/dukascopy.mjs`. Стакана там по-прежнему нет, но тики
> отдаются с bid/ask и объёмами (задержка 0.5–3 с).

### 🥇 Dukascopy freeserv (металлы + история 1m)
| Параметр | Значение |
|---|---|
| Тип данных | OHLCV свечи (1MIN…1MONTH) + тики bid/ask |
| Стоимость | **Бесплатно**, без ключа и регистрации |
| Инструменты | Мажоры, кроссы, **металлы (XAU/USD, XAG/USD)** |
| Глубина 1m | до 20 000 баров за запрос, архив — годы |
| Доступность из РБ | ✅ Проверено на прод-сервере |
| Минусы | Эндпоинт недокументированный; бары от 1h и старше отдаются только **закрытые** (текущий бар достраивается из минуток) |
