...
| **Week 2** | **≈ 45 %** | - Write unit tests for all functions in `src/lib/analytics/*` (metrics, positions, monteCarlo, materialize, exitEfficiency, exitAnalysis). ⟨ВЫПОЛНЕННО⟩ <br>- Add integration tests for the 5 largest routes (`/api/admin/collector/*`, `/api/orderflow/*`, `/api/stats/*`, `/api/liqmap/*`, `/api/econcal/*`). |
