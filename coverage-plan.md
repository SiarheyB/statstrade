...
| **Week 5‑5.5** | **≥ 90 %** | - Final sweep: close remaining `src/lib/*` 0 % gaps + edge cases. ⟨В ПРОЦЕССЕ⟩ <br>  ▸ Added 8 more lib test files (30 tests): `cache`, `mt-html`, `mt-parse`, `to-imported`, `deleteUser`, `exchangeToggle`, `featureConfig`, `google`. `src/lib` now **76 %** stmts (was 50 %), full suite **500 tests / 72 files, all green**. Found + documented a real `mt/parse` bug: `buildMt4` reads `exitTime` and `takeProfit` from the same column (`row[n-6-taxes]` vs literal `row[7]`). <br>  ▸ Added `coverage` block to `vitest.config.ts` (v8, text/json/html, `all:true`) with `exclude` for genuine infra (`collector/`, `scripts/`, `deploy/`, configs, `middleware.ts`, `instrumentation.ts`, mocks, `*.d.ts`, test files) so the gate reflects testable app code. <br>  ▸ **Blocker for the ≥90 % overall target (offline):** `src/app` pages (`page.tsx`, 8.9k stmts, 4 %) and `src/components` (5.3k stmts, 27 %) need full React rendering; `collector/` is a standalone deploy service. These require a live browser/DB (or Playwright E2E) — not achievable in the offline runner. The testable `src/lib` + route + component surface is the practical target. <br>- Remaining `src/lib` 0 % to close: `liqmap.ts`, `orderflow.ts` (prisma + candle/canvas heavy), `sync.ts` (20 %), `admin.ts` (43 %), `exchanges.ts` (48 %), `mentorShare.ts` (66 %), `analytics/materialize.ts` (73 %), `auth.ts` (74 %), `api.ts` (78 %). <br>- Verify four metrics on the scoped surface; commit + push the final suite when the testable surface reaches ≥90 %. <br>  ▸ **API route tests (Wave 2 — маршруты риск‑менеджера и авторизации).** ⟨ВЫПОЛНЕНО⟩ <br>  ▸ `risk` (GET): 5 тестов — 401 без сессии, пустой список аккаунтов, риск по нескольким аккаунтам, кастомные профили (override `accountId`), 500 при падении Prisma. Моки `@/lib/risk` (`parseRiskProfile`/`computeAccountRisk`) и `@/lib/analytics/positions`. <br>  ▸ `auth/login` (POST): 8 тестов — малформированный JSON (400), валидация Zod (missing/invalid email → 400), rate‑limit (429), неверные учётные данные (400), успешный вход без 2FA (200), 2FA‑флаг (200), 500 при ошибке БД. Моки `@/lib/auth`/`@/lib/ratelimit`/`@/lib/sync`. <br>  ▸ `auth/logout` (POST): 2 теста — успешный выход (200 `{ok:true}`), выход гостя. <br>  ▸ В `routeMocks.ts` добавлен `fill.findMany` (нужен роуту риска). <br>  ▸ **API route tests (Wave 3 — настройки, фичи, новости, биржи).** ⟨ВЫПОЛНЕНО⟩ <br>  ▸ `settings` (GET/PUT): 8 тестов — 401/дефолт/кастом, 400 малформированный JSON/валидация, дедуп + сохранение, 500 при ошибке Prisma. Мок `@/lib/db` (`prisma.user`). <br>  ▸ `features` (GET): 4 теста — 401 гость, 400 неизвестный ключ, 200 валидный ключ, 500 при throws. Мок `@/lib/featureConfig` (`getFeatureConfig`). <br>  ▸ `news` (GET): 2 теста — 401 гость, 200 список новостей. Мок `@/lib/news` (`getNews`; `asLang` оставлен реальным). <br>  ▸ `exchanges` (GET): 2 теста — 401 гость, 200 включённые биржи. Мок `@/lib/exchangeToggle` (`getEnabledExchangeMetas` — модуль целиком, т.к. тянет `ccxt`). <br>  ▸ В `routeMocks.ts` добавлены глобальные моки `@/lib/featureConfig`, `@/lib/news` (через `importActual`, чтобы `asLang` остался реальным), `@/lib/exchangeToggle` (без `importOriginal`, т.к. тянет `ccxt`). <br>  ▸ Полный прогон: **628 тестов / 90 файлов, все зелёные**. <br>  ▸ **API route tests (Шаг 3 — довести ВСЕ маршруты до ≥3 тестов).** ⟨ВЫПОЛНЕНО⟩ <br>  ▸ Группа A (auth, залита пользователем): `auth/register` (6), `auth/password` (8), `auth/2fa` (8), `auth/2fa/setup` (3), `auth/2fa/confirm` (5), `auth/2fa/login` (4), `auth/google` (5), `auth/google/link` (10), `cron/sync` (5). <br>  ▸ Группа B (accounts): `accounts` GET+POST (6), `accounts/[id]` PATCH+DELETE (6), `accounts/[id]/demo` (3), `accounts/[id]/import` POST+DELETE (7), `accounts/[id]/sync` (4), `account/delete` (4), `risk/settings` GET+PUT (6). Моки `@/lib/exchanges`, `@/lib/exchangeToggle`, `@/lib/statsCache`, `@/lib/crypto`, `@/lib/demo`, `@/lib/mt/parse`, `@/lib/mt/to-imported`, `@/lib/sync`, `@/lib/auth`, `@/lib/deleteUser`, `@/lib/risk`. <br>  ▸ Группа C (admin): `admin/accounts`, `admin/content` (4), `admin/donate` (3), `admin/donate/[id]`, `admin/errors`, `admin/errors/unread`, `admin/exchanges`, `admin/features` (6), `admin/users` (6), `admin/support` (3), `admin/support/[ticketId]` (7), `admin/support/unread` (3). <br>  ▸ Группа D (misc, залита пользователем): `annotations` (4), `admin/backup/file-info` (3), `admin/backup/upload` (4), `mt/preview` (4), `playbooks` (9), `share-links` (9), `support` (6), `support/[ticketId]` (10), `support/unread` (3), `trade-chart` (6), `trade-fills` (5). <br>  ▸ В `routeMocks.ts` дозалиты модели/экспорты: `exchangeAccount.create/findFirst/delete`, `importedTrade.findFirst/createMany/deleteMany`, `riskProfile.upsert/deleteMany`, `supportMessage.count/updateMany`, `errorLog.*`, `donateWallet.*`, `exchangeToggle.*`, `playbook.*`, `shareLink.*`, `supportTicket.*`, `supportMessage.*`, `$transaction`, `$queryRaw`, `mockExchangeToggle`, `asNonUser`. <br>  ▸ Полный прогон: **858 тестов / 129 файлов, все зелёные**. |
