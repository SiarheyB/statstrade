...
| **Week 2** | **≈ 45 %** | - Write unit tests for all functions in `src/lib/analytics/*` (metrics, positions, monteCarlo, materialize, exitEfficiency, exitAnalysis). ⟨ВЫПОЛНЕННО (кроме materialize)⟩ <br>- Add integration tests for the 5 largest routes (`/api/admin/collector/*`, `/api/orderflow/*`, `/api/stats/*`, `/api/liqmap/*`, `/api/econcal/*`). |
| **Week 3** | **≈ 70 %** | - Add component unit tests for 15 key UI pieces (`RiskBanner`, `TradeChart`, `AdminNav`, etc.). <br>- Implement Playwright scenarios for login → order‑flow and admin backup restore. |
