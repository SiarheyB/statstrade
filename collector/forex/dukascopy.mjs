// Клиент к бесплатному фиду Dukascopy (freeserv) — ОСНОВНОЙ источник свечей и
// тиков коллектора: и по валютным парам, и по металлам.
//
// Почему именно он:
...
//    Поэтому: вежливый темп опроса, ретраи на 5xx и резервный источник в
//    вызывающем коде — если Dukascopy замолчит, коллектор сам поднимает
//    Finnhub WS по валютным парам (см. collector/forex/fallback.mjs).

// Адрес выносится в ENV не ради гибкости, а ради проверяемости: подменив его
// на заведомо нерабочий, можно убедиться, что коллектор поднимает резервный
// источник (см. collector/forex/fallback.mjs). Тот же приём уже используется
// для Twelve Data (TWELVEDATA_API_BASE).
const BASE_URL = process.env.FX_DUKASCOPY_BASE_URL ?? "https://freeserv.dukascopy.com/2.0/index.php";
...
/**
 * Во сколько раз объём тика мельче объёма свечи.
 *
 * Проверено на живых данных (EUR/USD, USD/JPY, XAU/USD): сумма bidVol всех
 * тиков закрытой минуты, делённая на 1e6, совпадает с `v` минутного бара
 * до последнего знака (158880000 → 158.88; 34410 → 0.03441). То есть шкала
 * не «другая и несовместимая», как считалось раньше, а ровно миллионы
 * единиц — и минутку из тиков можно собирать вместе с объёмом.
 */
export const TICK_VOLUME_SCALE = 1e6;

/**
 * Последние тики (bid/ask с объёмами).
 *
 * Отсюда собирается ТЕКУЩАЯ (ещё не закрытая) минутка: закрытый 1MIN-бар
 * Dukascopy отдаёт только после закрытия минуты, а тики отстают на секунды
 * (замерено: 0.5–7 с). См. refreshFormingMinute в index.mjs.
 *
