...
## Форекс — три источника данных

- `collector/forex/` тянет свечи из **трёх** мест: тики мажоров — Finnhub WS,
  история мажоров — Twelve Data REST, **металлы (XAU/USD) и история 1m —
  Dukascopy** (`collector/forex/dukascopy.mjs`).
- **Золото есть только в Dukascopy:** у Finnhub его нет в WS, у Twelve Data оно
  проходит как commodity и на free-плане закрыто. Ключ Dukascopy не нужен, но
  **обязательны заголовки `User-Agent` и `Referer`** — без них приходит HTTP 200
  с пустым телом (выглядит как блокировка по гео, хотя дело не в ней).
- Какие инструменты идут через Dukascopy, задаёт `FX_DUKASCOPY_SYMBOLS`
  (по умолчанию `XAU/USD,XAG/USD`); они не подписываются на Finnhub и не
  расходуют кредиты Twelve Data.
...
- `exchange="finnhub"` в `FxCandle` — **исторический тег одного логического
  ряда, а не имя провайдера**: под ним лежат данные всех трёх источников,
  включая золото. Приложение фильтрует именно по этому значению, менять его
  нельзя без миграции данных.
