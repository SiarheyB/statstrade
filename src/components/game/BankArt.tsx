...
/** Левые края шести колонн — см. комментарий у их отрисовки ниже. */
const COLUMN_X = Array.from({ length: 6 }, (_, i) => 172 + i * 57);
  ...
  return (
    <svg viewBox="0 0 640 200" className="block w-full" role="img" aria-hidden>
      ...
      {/* Свет МЕЖДУ колоннами — пять промежутков на шесть колонн, а не один
          на колонну: светить за последней колонной, в пустоту после фронтона,
          и было той самой утечкой за рамку. */}
      {COLUMN_X.slice(0, -1).map((x, i) => (
        <rect
          key={`light-${i}`}
          x={x + 26}
          y="96"
          width={COLUMN_X[i + 1] - (x + 26) - 4}
          height="58"
          fill="var(--color-accent)"
          opacity={0.1 + health * 0.22}
        />
      ))}
