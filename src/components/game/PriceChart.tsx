    ...
    const draw = () => {
      ...
      if (pos) {
        ...
        if (xEntry >= layout.plotX - 10 && xEntry <= layout.plotX + layout.plotW + 10) {
          ...
          //
          // Раньше маркер рисовался с отступом в 14px ОТ ЛИНИИ входа — при
          // сильном приближении/отдалении графика та же дистанция в пикселях
          // соответствует то крошечной, то огромной разнице в цене, и кружок
          // выглядел так, будто вход был совсем на другом уровне. Маркер
          // должен сидеть точно НА линии входа при любом масштабе.
          drawTradeMarker(ctx, xEntry, yEntry, long, CHART_COLORS.accent);
        }
      }
