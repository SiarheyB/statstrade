...
type Cell = { date: string; r: number | null; winR: number; lossR: number; trades: number };
...
export default function RHeatmap({ daily }: { daily: DailyPoint[] }) {
  ...
  for (let t = start; t <= last; t += DAY_MS) {
    ...
    const netR = d ? d.winR + d.lossR : 0;
    cells.push({
      date,
      r: d && (d.winR !== 0 || d.lossR !== 0) ? netR : null,
      winR: d?.winR ?? 0,
      lossR: d?.lossR ?? 0,
      trades: d?.trades ?? 0,
    });
  ...
  const maxAbs = Math.max(
    1,
    ...cells.filter((c) => c.r !== null).map((c) => Math.abs(c.r!)),
  );

  function color(r: number | null): string {
    if (r === null) return "var(--color-surface-2)";
    if (r === 0) return "var(--color-border)";
    const intensity = 0.25 + 0.75 * Math.min(1, Math.sqrt(Math.abs(r) / maxAbs));
    return r > 0
          ...
          <div key={wi} className="flex flex-col gap-[3px]">
            {week.map((cell) => (
              <div
                ...
                style={{ backgroundColor: color(cell.r) }}
              />
            ))}
          </div>
        ))}
      ...
      {hover && (
        ...
        >
          ...
          {hover.cell.r === null ? (
            ...
            <>
              <div className="text-profit font-medium">{fmtR(hover.cell.winR)}</div>
              <div className="text-loss font-medium">{fmtR(hover.cell.lossR)}</div>
