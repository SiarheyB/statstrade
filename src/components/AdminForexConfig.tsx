...
// ─── Вкл/выкл раздела ───────────────────────────────────────────────────
//
// Хранится в общей таблице FeatureConfig (та же, что /admin/features) —
// это НЕ дублирующий источник данных, просто более удобный доступ рядом с
// остальными настройками форекса. Ключи: "forex" (общий выключатель, скрыт
// даже для админа + останавливает сбор данных в коллекторе) и
// "forexPublicAccess" (доступ для обычных пользователей, админ не затронут).

type AccessFeature = { key: "forex" | "forexPublicAccess"; label: string; description: string; value: { enabled: boolean } };
...
function AccessToggles() {
  ...
  const load = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      setFeatures(all.filter((f: AccessFeature) => f.key === "forex" || f.key === "forexPublicAccess"));
    }
  }, []);
  ...
  const forex = features.find((f) => f.key === "forex");
  const forexPublicAccess = features.find((f) => f.key === "forexPublicAccess");
