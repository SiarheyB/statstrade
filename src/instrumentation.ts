...
export async function register() {
  ...
  // Боты игры ходят своим циклом и НЕ гейтятся на ENABLE_SCHEDULER: на проде
  // он "false" (синхронизацию гоняет системный крон хоста), а мир игры должен
  // жить именно там. Выключается отдельно — GAME_BOTS_LIVE=false: это расход
  // на OpenRouter, и рубильник для него нужен свой.
  const { startBotLoop } = await import("./lib/game/botLoop");
  startBotLoop();

  // Прогрев главной: первый заход после старта контейнера иначе платит за
  // полный пересчёт (пять запросов в базу), а это как раз момент сразу после
  // деплоя. Считаем оба языка заранее — дальше страница всегда отдаётся из
  // памяти, включая моменты, когда кэш успел протухнуть.
  // Fire-and-forget: старт сервера не ждёт, ошибка не мешает подняться.
  import("./lib/landing")
    .then(({ getLandingData }) => Promise.all([getLandingData("ru"), getLandingData("en")]))
    .catch((err) => console.error("[landing-warmup] ошибка:", err));
