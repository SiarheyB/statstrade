...
export const useGameStore = create<GameStoreState>((set, get) => ({
  ...
  syncWorld: async () => {
    ...
    // Выигрыш на аукционе изъятого приходит той же дорогой, что и изъятие —
    // сервер решает исход торгов лениво, при первом же чтении витрины кем
    // угодно, а не обязательно самим победителем. Название вещи здесь не
    // переводим (как и при изъятии выше): у стора нет i18n, только счётчик.
    const won = result.data.wonItems ?? [];
    if (won.length > 0) {
      set((s) => ({
        game: {
          ...s.game,
          lifestyle: {
            ...s.game.lifestyle,
            ownedItemIds: [...new Set([...s.game.lifestyle.ownedItemIds, ...won])],
          },
        },
      }));
      get().notify("good", `Выиграно на аукционе изъятого: ${won.length}`);
    }
