...
// Ожидаемое число новостей на игровой день по всему рынку. Игровой день
// теперь равен реальному, поэтому 1.5 новости в сутки означало бы тишину:
// за получасовую сессию игрок не увидел бы ни одной. 24 — примерно одна в
// час, то есть за сессию одна-две, и это ровно тот ритм, при котором
// новостная лента живая, но не превращается в шум (частота настраивается в
// админке).
export const NEWS_PER_GAME_DAY = 24;

// Веса важности. Чёрный лебедь именно ЧЁРНЫЙ: положительных чёрных лебедей
// не бывает, все девять шаблонов этого уровня — катастрофы, поэтому их
// частота НАПРЯМУЮ превращается в долгосрочный нисходящий снос. Это не
// теория, а замеры: при весе 0.015 новости уносили цену в 0.001 от
// начальной за три года; при 0.001 (примерно полтора события в год) рынок
// терял около 0.3 годовой сигмы сверх заложенного сноса — ровно столько,
// сколько давал этот вес. 0.0003 — примерно одно событие в три года: риск
// обвала остаётся ощутимым, но не съедает весь рост.
