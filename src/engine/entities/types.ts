...
export interface Position {
  ...
  botId?: string;
  /**
   * Чей сигнал скопирован. Нужна, чтобы при закрытии в плюс заплатить
   * ведущему его долю: без метки скопированная сделка неотличима от своей.
   */
  copiedFrom?: string;
  /**
   * Сигнал, с которого списана сделка.
   *
   * Комиссия ведущему начисляется по нему: сервер берёт из сигнала автора и
   * ставку подписки и не даёт заплатить по одному сигналу дважды. По одному
   * лишь ведущему это было неотличимо от выдуманной сделки.
   */
  copiedSignalId?: string;
  /** Комиссия ведущего, % — фиксируется в момент копирования. */
  copyFeePct?: number;
  openedAt: number;
  closedAt?: number;
  closePrice?: number;
...
export interface LifestyleState {
  ...
  unpaidUpkeep: number; // сколько не смогли списать (баланс кончился) — это ДОЛГ, а не забытая цифра
  /**
   * Когда долг по содержанию появился впервые, мс реального времени.
   *
   * От неё считается льготный срок: пока он идёт, вещь можно продать самому
   * и дороже. Погасил долг — отметка снимается.
   */
  upkeepDebtSince?: number | null;
}
...
export interface CareerState {
  /** Основное место работы. Оно всегда одно. */
  job: JobState | null;
  /**
   * Подработка. Совмещать с основной работой можно только её — курьера.
   *
   * Старые сохранения поля не знают, поэтому оно необязательное.
   */
  sideJob?: JobState | null;
