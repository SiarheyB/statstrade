...
export const FEATURE_DEFAULTS = {
  ...
  newsFeed: {
    label: "Новости (лента крипто-СМИ)",
    description:
      "Раздел «Новости» в меню пользователя: лента заголовков из бесплатных RSS крипто-изданий (для русского языка — ForkLog, BeInCrypto, Incrypted; для английского — CoinDesk, Cointelegraph, Decrypt). Приложение само ходит на эти сайты за списком статей, сохраняет заголовки в свою базу и показывает их пользователям; сами статьи открываются на сайте издания. Если выключить этот переключатель — пункт «Новости» пропадает у всех пользователей, страница перестаёт открываться, а приложение перестаёт ходить за новостями и чистить старые (уже сохранённые записи при этом НЕ удаляются, лента просто «замораживается»). Экономический календарь — отдельная функция, этот переключатель на него не влияет.",
    fieldHelp: {
      retentionDays:
        "Через сколько дней удалять новости из базы. Например 2 — в ленте остаются только статьи, опубликованные за последние двое суток, всё более старое удаляется НАВСЕГДА при очередном обновлении ленты (примерно раз в 15 минут, пока кто-то заходит в раздел). Меньше — лента свежее и база компактнее; больше — глубже архив, но таблица новостей растёт (сотни статей в день) и страница грузится медленнее. 0 или пусто = ничего не удалять (не рекомендуется: чистки новостей больше нигде нет). Разумный диапазон: 1–7.",
    },
    // Сколько дней держать новости в БД; 0 = не удалять.
    retentionDays: 2,
  },
  econcalFeed: {
    label: "Экономический календарь",
    description:
      "Раздел «Экономический календарь» в меню пользователя: расписание макро-событий (заседания ФРС, отчёты по инфляции и занятости и т.п.) с прогнозом, предыдущим и фактическим значением. Приложение бесплатно забирает расписание текущей недели с ForexFactory/faireconomy и обновляет его примерно раз в 30 минут, дописывая фактические значения по мере выхода данных. На странице пользователь видит ТЕКУЩУЮ неделю, листать назад нельзя. Если выключить переключатель — пункт меню пропадает у всех, страница перестаёт открываться, расписание не обновляется и старые события не удаляются. Лента новостей — отдельная функция, этот переключатель на неё не влияет.",
    fieldHelp: {
      retentionDays:
        "Через сколько дней удалять ПРОШЕДШИЕ события из базы. Удаление происходит при каждом обновлении расписания (примерно раз в 30 минут, пока кто-то заходит в раздел): всё, что было раньше указанного срока, стирается НАВСЕГДА, а сегодняшние и будущие события не трогаются никогда. Не ставь меньше 7: страница показывает неделю целиком, и при значении 2 в воскресенье начало недели окажется пустым. Больше — глубже архив, но таблица растёт и страница грузится медленнее. 0 или пусто = не удалять (не рекомендуется: другой чистки календаря нет). Разумный диапазон: 7–30.",
    },
    // Сколько дней хранить прошедшие события; 0 = не удалять.
    retentionDays: 7,
  },
  imbalanceIndicator: {
