...
export const FEATURE_DEFAULTS = {
  ...
  game: {
    ...
    description:
      "Полностью отключает раздел «Игра» (симулятор трейдинга): пункт меню пропадает даже у админа, страница /dashboard/game отвечает «доступ запрещён». Движок игры целиком клиентский (без сервера/БД — прогресс лежит в IndexedDB браузера игрока), поэтому выключение не трогает ничьи сохранения, просто скрывает раздел. Числа ниже — настройки баланса игры: они приезжают к игроку при загрузке страницы, поэтому применяются сразу после перезагрузки, БЕЗ передеплоя и без сброса чужого прогресса.",
    fieldHelp: {
      startingBalance:
      ...
      newsPerGameDay:
        "Сколько новостей в среднем выходит за ОДИН игровой день. Каждая новость двигает цену затронутых бумаг и повышает их волатильность на некоторое время. 0 — новостей нет вообще (чистая случайная цена), 1.5 — примерно как на реальном рынке, 5+ — новостной шторм, торговать по графику почти невозможно. Разумный диапазон: 0–4.",
      blackSwanPerMille:
