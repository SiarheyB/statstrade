...
import { runBackupScript } from "@/lib/backrunner";

const PROJECT_ROOT = process.cwd();
const TMP_DIR = join(PROJECT_ROOT, "backup", "tmp");
const LOG_FILE = join(PROJECT_ROOT, "backup", "db-backup-functions.log");

// in-memory store for ops (same as backrunner.ts)
let operations = {} as Record<string, any>;
...
export async function POST(request) {
  try {
    ...
    if (action === "schedule") {
      ...
      // Read user timezone from cookie
      const { getTimezone } = await import("@/lib/i18n/server");
      ...
      // Restart backup scheduler with new config and timezone
      console.log("[backup-scheduler] Schedule updated, restarting scheduler");
      const { restartBackupScheduler, setUserTimezone } = await import("@/lib/backup-scheduler");
    ...
    // Run backup script via shared runner (no HTTP, no auth middleware)
    const returnedOpId = await runBackupScript(action, file);

    return new Response(JSON.stringify({ operationId: returnedOpId }), { headers: { "Content-Type": "application/json" } });
  } catch (err) {
    console.error("[backup‑route] Unexpected error:", err);
