...
// Окно наложений выровнено вниз до минуты (см. GET), поэтому в пределах минуты
// запрос буквально один и тот же — кэшируем его на всю минуту. Прежние 3000 мс
// при опросе LIVE раз в 3 секунды почти всегда промахивались, и тяжёлая
// агрегация запускалась заново на каждый тик.
const TTL_MS = 60_000;
const ALIGN_MS = 60_000;
...
export async function GET(req: Request) {
  ...
  // Выравниваем вниз до минуты: rollup-бакеты минутные, точнее правого края
  // карта всё равно не знает, а стабильная граница делает окно одинаковым для
  // всех запросов внутри минуты — только так работают кэш и ETag.
  toMs = Math.floor(toMs / ALIGN_MS) * ALIGN_MS;
  ...
  // `to` уже выровнен по минуте, поэтому в ключе он безопасен: внутри минуты
  // значение одно, а на следующей минуте окно и правда другое.
  const key = `${symbol}|${exchange}|${range}|${timezone ?? "none"}|${withCandles ? 1 : 0}|${toMs}`;
  try {
