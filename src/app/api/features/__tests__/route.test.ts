...
describe("GET /api/features", () => {
  beforeEach(() => {
    ...
    vi.mocked(require("@/lib/featureConfig").getFeatureConfig).mockReset();
  });
  ...
  it("returns feature config for valid key", async () => {
    asUser();
    vi.mocked(require("@/lib/featureConfig").getFeatureConfig).mockResolvedValue({ enabled: true });
    const res = await GET(new Request(`${base}?key=playbooks`));
  ...
  it("returns 500 when getFeatureConfig throws", async () => {
    asUser();
    vi.mocked(require("@/lib/featureConfig").getFeatureConfig).mockRejectedValueOnce(new Error("DB error"));
    const res = await GET(new Request(`${base}?key=playbooks`));
