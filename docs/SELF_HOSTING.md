...
2. **Платформы**: отметить «Веб-сервисы», указать Redirect URI:
   `https://<ваш-домен>/api/integrations/yandex-disk/callback`.
3. **Доступ (Scopes)**: выбрать **«Яндекс.Диск REST API: доступ к папке
...
4. Скопировать **ID** и **Пароль** приложения (Client ID / Client Secret) в
   `.env`:
   ```
