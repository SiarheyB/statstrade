...
### 9.4 Push-уведомления (системный крон, раз в минуту)
Одна задача обслуживает оба вида уведомлений, приходящих при **закрытой вкладке**:

* **«цена подошла к уровню»** — на странице «Рекомендации» у каждого уровня есть колокольчик;
  человек подписывается, и push приходит, когда цена окажется ближе заданного расстояния
  (в долях ATR, по умолчанию 0.25);
* **«скоро выходит новость»** — те же напоминания экономического календаря, что рисует
  всплывашку в открытой вкладке; настройки берутся из копии, которую устройство присылает
  при их изменении (в самом браузере они по-прежнему главные, см. `/dashboard/settings`).
```bash
( crontab -l 2>/dev/null; \
  echo '* * * * * curl -fsS --max-time 50 -H "Authorization: Bearer $(grep -E ^CRON_SECRET= ~/statstrade/.env | cut -d= -f2)" http://127.0.0.1:3000/api/cron/alerts >/dev/null 2>&1' \
) | crontab -
```
Раз в минуту — это не опечатка и почти ничего не стоит: за один проход делается **один**
запрос к Binance за ценами ВСЕХ бессрочных контрактов сразу, независимо от того, сколько
инструментов отслеживают пользователи. Если активных подписок нет вообще, наружу не уходит
и он. Реже нельзя по сути задачи: уровень — это цена, у которой сделка либо случается, либо
нет, и уведомление через пять минут после подхода уже бесполезно.

Для отправки нужны ключи **VAPID** в `.env` (см. `.env.example`, раздел «Push-уведомления»):
```bash
npx web-push generate-vapid-keys
```
> Ключи менять нельзя: подписки браузеров привязаны к публичному ключу и после смены пары
> молча перестают работать. Без ключей приложение работает как раньше — просто не предлагает
> подписку (карточка в настройках не рисуется, колокольчик пишет «не настроено на сервере»).
>
> Push требует **https** (или localhost): по незащищённому http браузер не даст
> зарегистрировать service worker. На проде это обеспечивает Tailscale Funnel.
...
```
Ответ вида
`{"ok":true,"levels":{"checked":3,"fired":0,"rearmed":0,"pushed":0},"econcal":{"devices":1,"sent":0,"removed":0}}`.
В `levels`: `checked` — сколько подписок на уровни просмотрено, `fired` — по скольким
отправлено уведомление, `rearmed` — сколько сработавших ранее заряжено заново (цена отошла
от уровня), `pushed` — на сколько устройств реально ушёл push. В `econcal`: `devices` —
сколько устройств с включёнными напоминаниями, `sent` — сколько уведомлений отправлено,
`removed` — сколько мёртвых подписок удалено по дороге.

> Два прохода независимы: упавший календарь не отменяет проверку уровней и наоборот.

### 9.5 Такт ботов игры
Боты в разделе «Игра» ходят **сами**: контейнер `app` держит собственный цикл (`startBotLoop`,
запускается из `src/instrumentation.ts`), и раз в пять минут каждый бот смотрит на рынок и
решает, что делать. Это работает и тогда, когда в игру никто не играет — мир, который замирает
без зрителя, читается как декорация. Никакой настройки для этого не нужно: цикл поднимается
вместе с контейнером.

> **Это расход на OpenRouter, идущий круглые сутки.** Часть решений боты принимают моделью
> (доля задаётся ползунком «глубина ИИ» в `/admin/game` → «Боты»). Не хотите платить за ночь —
> поставьте глубину ИИ в 0 (встроенная логика работает без сети и бесплатно) или выключите цикл
> целиком переменной `GAME_BOTS_LIVE=false` в `.env`.

Цикл не гейтится на `ENABLE_SCHEDULER` (на проде он `false`, потому что синхронизацию гоняет
системный крон хоста): у ботов свой рубильник, потому что это отдельные деньги.

Отдельно есть эндпоинт `/api/cron/game-bots` с тем же секретом, что у остальных задач. Он нужен
только там, где процесс приложения НЕ живёт постоянно (serverless и подобное) — при обычном
самохостинге в Docker он не требуется:
```bash
curl -s -H "Authorization: Bearer $(grep -E ^CRON_SECRET= ~/statstrade/.env | cut -d= -f2)" \
  http://127.0.0.1:3000/api/cron/game-bots
```
...
### 11.1 Обложки новостей хранятся у нас

Источники отдают обложку в исходном размере со своего CDN — замер на проде:
1.5 МБ и 16.7 секунды на одну картинку, показываемую плашкой в 80 px. Поэтому
приложение при обходе фидов скачивает обложку один раз, ужимает в webp шириной
320 px (получается 5–25 КБ) и кладёт в таблицу `NewsImage`; страница берёт её
у нас — `/api/news/image/<id>`, кэш на год.

- Порция за обход — 12 картинок, по три параллельно; не скачалось — карточка
  остаётся со ссылкой на внешний CDN, следующий обход попробует снова.
- В логах приложения: `[news] обложек сохранено 12, не удалось 0`.
- Место: тысяча новостей ≈ 20 МБ, попадает в обычный бэкап базы.
- Чистить вручную не нужно: у `NewsImage` внешний ключ с `ON DELETE CASCADE`,
  и обложка удаляется вместе с новостью, когда её убирает ретеншн ленты.

### 11.2 VACUUM: коллектор убирает мусор обновлений сам
