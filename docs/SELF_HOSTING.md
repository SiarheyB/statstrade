...
### 9.4 Такт ботов игры (системный крон, необязательный)
Боты в разделе «Игра» ходят лениво: их такт дёргается на запросах котировок, мира и чата.
Для одного игрока за вечер этого достаточно, но у ленивой схемы есть провал — ночь. Человек
заходит утром и видит рынок, прошедший за десять часов половину графика, и ботов, которые всё
это время сидели в тех же позициях. Крон закрывает этот провал:
```bash
( crontab -l 2>/dev/null; \
  echo '*/10 * * * * curl -fsS --max-time 120 -H "Authorization: Bearer $(grep -E ^CRON_SECRET= ~/statstrade/.env | cut -d= -f2)" http://127.0.0.1:3000/api/cron/game-bots >/dev/null 2>&1' \
) | crontab -
```
Задача необязательная: без неё игра работает по-прежнему, просто мир шевелится только при
живых игроках. Частота выше «раз в десять минут» смысла не имеет — сам бот всё равно ходит
не чаще раза в пять минут.

> **Это расход на OpenRouter.** Часть решений боты принимают моделью (доля задаётся ползунком
> «глубина ИИ» в `/admin/game` → «Боты»). Крон означает, что запросы идут круглые сутки, а не
> только пока кто-то играет. Не хотите платить за ночь — не ставьте задачу или поставьте
> глубину ИИ в 0: встроенная логика работает без сети и бесплатно.
...
- [ ] (необязательно) Такт ботов игры в cron (`/api/cron/game-bots`, см. §9.4).
