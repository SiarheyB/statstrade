...
- ⚠️ `npm audit`: **6 high + 2 moderate**. Четыре (`next`, `postcss`, `sharp`,
  `undici` частично) закрываются обновлением **next до 16.3.0** — среди них
  «Middleware / Proxy bypass in App Router» в самом Next. Отдельным пунктом.
...
- Оставшиеся 148 — рассинхрон типов моков с прод-типами (см. ниже), тесты не
  исключены из `include` в `tsconfig.json`. `next build` в Docker выполняет
  тайпчек по этому же tsconfig — стоит проверить, не проходит ли сборка только
  по случайности.
...
7. [ ] Обновить `next` до 16.3.0 — закрывает 4 из 6 high-уязвимостей
   `npm audit`, включая Middleware/Proxy bypass в App Router.
...
11. [~] B5 решён, фундамент готов; остался сам «Календарь».
    ...
    **Осталось:** `GET /api/calendar` поверх `TradeHourly` + перевод страницы
    «Календарь» на `exitTime` и серверную агрегацию.
