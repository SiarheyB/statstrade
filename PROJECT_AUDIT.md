...
11. [ ] Решить B5 (`entryTime` vs `exitTime` + таймзона) и только после этого —
    `GET /api/calendar` поверх агрегатов: сейчас «Календарь» нарезает дни иначе
    и читать `TradeDaily` не может.
